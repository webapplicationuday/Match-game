...
class ImageGame extends Component {
  ...
  renderWinOrLoosCard = () => {
    ...
    return <WinOrLoseCard score={topScore} playAgain={this.playAgain} />
  }

  renderOfImagesList = () => {
    const {showedImage, activeTabId} = this.state
    const {imageUrl} = showedImage
    const filteredImages = this.getFilteredImagesList()
    return (
  ...
  render() {
    const {topScore, isGameStarted, time} = this.state
    return (
      <div className="app-container">
        ...
        <div className="images-card">
          {isGameStarted
            ? this.renderOfImagesList()
            : this.renderWinOrLoosCard()}
        </div>
      </div>
    )
  }
